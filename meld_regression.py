...
#%%
df=pd.read_excel('C:/Users/aurel/OneDrive/Bureau/HAA_Dataset_final_2_meld.xlsx');
...
modele_regLog = linear_model.LogisticRegression(penalty='l2',class_weight=None ,solver='lbfgs',max_iter=1000000, n_jobs=-1, random_state=0);
...
modele_regLog.fit(x_train, ytrain); #cv= 6-fold cross validation

with open('datasaving.txt', mode='a') as file:
    file.write("\nCoeff Regression Logistique:");
    file.write(str(modele_regLog.coef_))

    
#%%

survival_prediction=modele_regLog.predict(x_test);
survival_predictions_proba = modele_regLog.predict_proba(x_test);
...
plt.show();
#précision du modèle
acc = modele_regLog.score(x_test,y_test);
print('\nGlobal Accuracy                       : ');
print(acc*100);
with open('datasaving.txt', mode='a') as file:
...
display.plot(ax=ax1, color="red", label="No_Normalization_and_No_PCA");
