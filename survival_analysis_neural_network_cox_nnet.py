...
class HiddenLayer(object):
    def __init__(self, rng, input, n_samples, map, label, activation=T.tanh):
        ...
        for i in range(len(self.map)):
            output[i] = self.activation(T.dot(input_cat[i], self.W[i]) + self.b[i])
...
#%%
#splitting the data 
x_train, x_test, ytime_train, ytime_test, ystatus_train, ystatus_test = train_test_split(x, T , E, 
...
cv_params = dict(cv_seed=1, n_folds=6, cv_metric = "cindex",
    L2_range = numpy.arange(-5,-1))
...
#%%
#Build the model
L2_reg = L2_reg_params[numpy.argmax(mean_cvpl)] #on choisit la regularisation qui donne la plus grande moyenne de partial loglikelihooh en cross validation
...
#%%
#predict on new data
theta2=model.predictNewData(x_train)

#%%

Cindex_train=CIndex(model,x_train,ytime_train, ystatus_train);
print('CIndex of the Test Dataset', Cindex_train)
#%%
