...
n_units1=[5,20,5,10,5,15]
n_units2=[20,5,10,5,15,5]
#n_units1=[5,19,5,17,5,18]
#n_units2=[19,5,17,5,18,5    ]
...
for n1,n2 in zip(n_units1,n_units2):
    
    
    for drop_out1 in drop_out_values:
        
        for drop_out2 in drop_out_values:
            ...
            for train2_index, val_index in index:
                ...
                model.add(tf.keras.layers.Dense(units=n1,kernel_initializer=initializer, activation= 'relu'))
                model.add(tf.keras.layers.Dropout(drop_out1))
                model.add(tf.keras.layers.Dense(units=n2,kernel_initializer=initializer , activation='relu'))
                ...
                # = os.path.join(working_dir, 'models_saved_MLP', file_name(fold_var))
            ...
            if mean_val_bal_acc > ref_bal_acc:
                ...
                best_n1_units=n1
                best_n2_units=n2
                best_drop_out1= drop_out1
                best_drop_out2=drop_out2
...
#model.add(tf.keras.Dropout(0.8))
model_opti.add(tf.keras.layers.Dense(units=best_n1_units,kernel_initializer=initializer , activation= 'relu'))
model_opti.add(tf.keras.layers.Dropout(best_drop_out1))
model_opti.add(tf.keras.layers.Dense(units=best_n2_units,kernel_initializer=initializer , activation= 'relu'))
model_opti.add(tf.keras.layers.Dropout(best_drop_out2))
