...
#cutpoints= [1.0,15.0,30.0,45.0,60.0,75.0,90.0,105.0,120.0]
...
#number_nodes=[[5,5],[10,10],[15,15],[20,20]]
node_hidden_layer_1=[5,20,5,10,5,18]
node_hidden_layer_2=[20,5,10,5,18,5]
#num_nodes = [32, 32]
batch_norm = True
...
for num_nodes1,num_nodes2 in zip(node_hidden_layer_1, node_hidden_layer_2):

    for dropout1 in dropout_values:
        
        for dropout2 in dropout_values:
            ...
            #Make neural network
                ...
                layer1=torch.nn.Linear(in_features, num_nodes1)
                layer2=torch.nn.Linear(num_nodes1, num_nodes2)
                layer3=torch.nn.Linear(num_nodes2, out_features)
                
                net = torch.nn.Sequential(
                layer1,
                torch.nn.ReLU(),
                torch.nn.BatchNorm1d(num_nodes1),
                torch.nn.Dropout(dropout1),
                
                layer2,
                torch.nn.ReLU(),
                torch.nn.BatchNorm1d(num_nodes2),
                torch.nn.Dropout(dropout2),
                
                layer3
                )
                torch.nn.init.kaiming_normal_(layer1.weight, nonlinearity='relu')
                torch.nn.init.kaiming_normal_(layer2.weight, nonlinearity='relu')
                #torch.nn.init.kaiming_normal_(layer3.weight, nonlinearity='relu')
            ...
            print('Time dependant c-index over 6-fold cross-validation',c_index_vec)
            print('Mean td c-index over the 6-fold', statistics.mean(c_index_vec))
            
            c_index_mean_vec.append(statistics.mean(c_index_vec))
            print('Integrated Brier Score over 6-fold cross-validation', integrated_brier_score_vec)
            print('Mean Integrated brier Score over the 6-fold', statistics.mean(integrated_brier_score_vec))
...
in_features = x_train.shape[1]
out_features = labtrans.out_features
layer1=torch.nn.Linear(in_features, n1)
layer2=torch.nn.Linear(n1, n2)
layer3=torch.nn.Linear(n2, out_features)

net_opti = torch.nn.Sequential(
layer1,
torch.nn.ReLU(),
