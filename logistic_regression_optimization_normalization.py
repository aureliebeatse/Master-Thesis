...
cols_normalize = ['Weight_Kg', 'Size_cm', 'Age_(yr)', 'INR_D0', 'Bilirubine_D0','Creatinine_D0', 'albumine_D0', 'WBC']
cols_leave = ['Sex', 'Ascitis', 'Encephalopathy']

normalize = [([col], MinMaxScaler()) for col in cols_normalize]
...
grid_search= GridSearchCV(modele_regLog, param_grid, scoring=scores,refit='balanced_accuracy',n_jobs=-1,cv=skfold2.split(x_train, ytrain), return_train_score=True);
...
display.plot(ax=ax1, color="red", label="Normalization_and_No_PCA");


ax=plt.gca()
col_labels=['AUC','AUC CI (95%)']
row_labels=['Normalization and No PCA']
