...
with open('datasavingrf.txt', mode='w+') as file:
...
param_grid = [{'n_estimators': forest_n_estimators,'criterion': ['gini'],'max_depth': [None,3,6,8], 'max_features': [3,4,5,6,7,8,9,10,11], 'bootstrap':[False]}];
...
grid_search= GridSearchCV(forest, param_grid, scoring='balanced_accuracy',n_jobs=-1,cv=skfold2.split(x_train, ytrain), return_train_score=True);
grid_search.fit(x_train, ytrain); #cv=3 3-fold cross validation

#%%

print('Best number of trees:', grid_search.best_estimator_.get_params()['n_estimators'])
print('Best maximum depth:', grid_search.best_estimator_.get_params()['max_depth'])
print('Best criterion:', grid_search.best_estimator_.get_params()['criterion'])


RF_parameters=grid_search.best_estimator_.get_params();
best_parameters=RF_parameters
...
best_forest.fit(x_train,ytrain);

#%%
CV_Results= cross_val_score(best_forest, x_train,ytrain,scoring='balanced_accuracy', cv=skfold2.split(x_train, ytrain), n_jobs=-1); #n_jobs=-1 means using all processors available
...
survival_prediction=best_forest.predict(x_test);
survival_predictions_proba = best_forest.predict_proba(x_test);
...
display.plot(ax=ax1, color="red", label="Standardisation_and_no_PCA");
...
plot_tree(best_forest.estimators_[2], 
          feature_names=x_mapper.transformed_names_,
...
cols=x_mapper.transformed_names_
colls=[]
for s in sort:
    colls.append(cols[s])
